...
def chat(request):
    ...
    return render(request, 'chat_test.html', context)
...
def upload_file(request):
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        # 处理文件
        try:
            content = agent.process_file(file_path, file.content_type, user_id)

            # 获取AI回复
            response = agent.chat_agent(f"我上传了一个{file.content_type}类型的文件，内容是: {content[:100]}...",
                                        user_id)
