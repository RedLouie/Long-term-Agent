...
class LongTermMemoryAgent:
    def __init__(self, db_path="stu_memory3.db"):
        self.client = OpenAI(api_key=ljy_key,
                             base_url="https://api.chatanywhere.tech/v1")
        self.db_path = db_path
        # 配置参数
        self.embedding_model = "text-embedding-3-small"
        # self.chat_model = "gpt-3.5-turbo-1106"
        self.chat_model = "gpt-4.1-2025-04-14"
    ...
    def chat_agent(self, user_input: str, user_id: int) -> str:
        ...
        system_prompt = f"""
        首先，你要做的是生成英文回复以及中文的翻译
        你是一个英语教学专用提问式AI智能体。
        核心使命：通过苏格拉底式提问激发学生思政表达（基于历史对话：{context}以及当前对话记录：{recent_messages}）
        让学生对特定思辨问题进行回答

        教学系统功能说明：
        1. 基于教学单元自动生成文化对比情境卡
        2. 基于单元知识点自动生成冲突场景(如“中秋送礼被误认为贿赂")
        3. 用中英结合进行对话练习
        4. 实时分析学生语言能力与文化理解度
        5. 动态构建学生个人学习档案
        ...
        4. 实时文化注解:对话中自动弹出背景知识卡片(如"西方圣诞袜起源 vs 中国压岁钱典故")
        5. 对话示例：
           ...
           [这个问题问得好！关键的传统是……顺便问一下，你能说出一个具有类似家庭团聚意义的西方节日吗？]"
        ...
        metadata_response = self.client.chat.completions.create(
            ...
            **成长档案生成**:记录跨文化交际典型错误与改进建议(如"避免文化刻板印象表述")
            **综合得分**：62/100（CEFR A2）
            **提升计划**：
            - 主谓一致专项训练（错误率降低目标：70% → 90%）
            - 医疗话题词汇拓展"""},{"role":"user","content":f"这是历史的教学记录{context}"}]
